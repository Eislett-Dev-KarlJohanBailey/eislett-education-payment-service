  ...
  try {
    ...
    for (let i = 0; i < billingEvents.length; i++) {
      ...
      try {
        ...
        console.log(`Successfully processed dunning event: ${billingEvent.type} (${billingEvent.meta.eventId})`);
      } catch (error) {
        console.error(`Failed to process dunning event ${billingEvent.type} (${billingEvent.meta.eventId}):`, error);
