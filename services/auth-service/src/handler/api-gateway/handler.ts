...
export async function apiHandler(event: APIGatewayProxyEvent) {
  try {
    ...
    // Auth endpoints don't require JWT (they generate tokens)
    // But update and /me endpoints do require authentication
    const isAuthEndpoint = normalizedPath.includes("/auth/google");
    let user: { id: string; role?: string } | null = null;
    
    if (!isAuthEndpoint) {
      // For update and /me endpoints, require authentication
