...
export const handler = async (
  ...
  try {
    ...
    // Process each event (batch of up to 20)
    for (let i = 0; i < billingEvents.length; i++) {
      const billingEvent = billingEvents[i];
      const sqsRecord = event.Records[i];

      try {
        // Process the billing event and record transaction
        await processBillingEventUseCase.execute(billingEvent);

        console.log(`Successfully processed transaction event: ${billingEvent.type} (${billingEvent.meta.eventId})`);
      } catch (error) {
        console.error(`Failed to process transaction event ${billingEvent.type} (${billingEvent.meta.eventId}):`, error);
