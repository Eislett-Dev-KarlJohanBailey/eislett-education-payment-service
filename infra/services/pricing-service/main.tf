...
# DynamoDB Table for Prices
resource "aws_dynamodb_table" "prices" {
  name         = "eislett-education-${var.environment}-prices"
...
module "pricing_service_iam_role" {
  source = "../../modules/lambda_iam_role"

  role_name = "pricing-service-lambda-role-${var.environment}"
...
module "lambda_api_link" {
  ...
  lambda_function_arn  = module.pricing_service_lambda.function_arn
  lambda_function_name = module.pricing_service_lambda.function_name
  paths = ["prices"]
}
